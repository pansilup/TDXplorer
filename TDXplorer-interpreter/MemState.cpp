...
bool SYMemState::readSymbolicValue(ulong addr, ulong size, SymCellPtr &SymList) {
    ...
    ulong hcave_sz = addr - aligned_addr;
    if (hcave_sz + size > SYMEM_BLOCK_SIZE) {
        // FIX_ME();  // Doesn't support cross-chunk memory read: revisit and check if this is already fixed.
    }
...
bool SYMemState::readSymbolicValue(ulong addr, ulong size, KVExprPtr &e) {
    ...
    ulong hcave_sz = addr - aligned_addr;
    if (hcave_sz + size > SYMEM_BLOCK_SIZE) {
        // FIX_ME();  // Doesn't support cross-chunk memory read, check again if this is fixed
    }
    SYMemChunk *c = m_Bitmap.findSYMemChunk(aligned_addr);
    assert(c != NULL);
...
bool SYMemState::sendSeamMemAccessReq(SERVREQ req, ulong seam_va, long &data, ulong size){
    ...
    while(sreq->req_owner == SERVREQ_OWNER_S_AGENT){
        /*wait*/
        // std::cout << "SERVREQ_OWNER_S_AGENT\n";
        asm volatile ("mfence; \n");
    }
    ...
    // std::cout << "SERVREQ_OWNER_INTERPRETER\n";
