...
int CAnalyze::onEndOfInsExec(){ //analysis at the end of each instruction
    ...
    if(to_add_new_symbol == 1){
        ulong tdcs_attributes_adr = tdcs_start_seam_va + sreq->tdcs_attributes_offset;
        ulong tdcs_atributes;

        assert(tdcs_start_seam_va != 0);
        MemValue MV{tdcs_attributes_adr, 0x8};
        bool res = m_VM->readMemory(MV);
        assert(res);
        assert(!MV.bsym);

        tdcs_atributes = MV.i64;
        std::cout << "tdcs_atributes: 0x" << tdcs_atributes << std::endl;
        m_VM->createSYMemObject(tdcs_attributes_adr, 8, 1, 1,tdcs_atributes,"tdcs_atributes");
