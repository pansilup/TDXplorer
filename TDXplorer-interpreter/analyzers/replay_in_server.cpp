...
int CAnalyze::onBeforeCIESIE(){
    ...
    for(i = 0; i < 2; i++){
        ...
        if(execData->opDetails[i].opmemac.memrdwr){
            ...
            if((adr > 0xffffa00300221078) && adr < (0xffffa00300221078 + 0x4004)){
                std::cout << "KOT access: 0x" << std::hex << adr << std::endl;
            }
...
bool CAnalyze::beginAnalysis(ulong addr) { //Analysis start
    ...
    ulong keyid = m_regs->regs.rdx;
    m_regs->regs.rdx= 0x20;
    // std::cout << "keyid: 0x" << std::hex << keyid << std::endl;
    // m_VM->createSYRegObject(x86_64::rdx, 8, 1, 1, keyid, "keyid");

    // bool res;
    // MemValue mv;
    // mv.size = 8;
    // mv.addr = 0xffffa00300221188;
    // mv.bsym = true;
    // mv.isSymList = false;
    // res = m_VM->readMemory (mv);
    // assert(res);
    // assert(!mv.bsym);
    // m_VM->createSYMemObject(mv.addr, 8, 1, 1, mv.i64, "kot");


    // m_regs->regs.rdx = 0x27; //seamcall failing value
    // m_regs->regs.eflags &= ~(1UL << 8); //stop single stepping
