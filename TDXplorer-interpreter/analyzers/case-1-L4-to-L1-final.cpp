...
int CAnalyze::onEndOfInsExec(){ //analysis at the end of each instruction
    ...
    if(execData->insn_count == 2){
        ...
        for (auto it : m_VM->m_SYMemObjects)
        {
            ...
            ulong v;
            // memcpy((void *)&v, (void *)(&it.second->i64), 8);
            std::cout << "s name :" << it.second->name << " ,hasseed : " << it.second->has_seed  << std::endl;
            switch(it.second->size)
            {
                case 1:
                    v = (ulong)it.second->i8 ;
                    std::cout << "val:0x " << std::hex << v << std::endl;
                    break;
                case 4:
                    v = (ulong)it.second->i32 ;
                    std::cout << "val:0x " << std::hex << v << std::endl;
                    break;
                case 8:
                    v = (ulong)it.second->i64 ;
                    std::cout << "val:0x " << std::hex << v << std::endl;
                    break;
            }
        }
    ...
    // if((updated_sept_page_seam_va != 0) && !is_epte_defined){
    //     ulong epte_seed = 0x80e0000000000000;
    //     // td_pml5_seam_va = td_pml5_seam_va_2;
    //     // updated_sept_page_seam_va = td_pml5_seam_va;
    //     std::cout << "considered td_pml5_seam_va: 0x" << updated_sept_page_seam_va << std::endl;
    //     m_VM->createSYMemObject(updated_sept_page_seam_va + 0x8, 8, 1, 1, epte_seed, "epte");
    ...
    // if(execData->insn_count == 2457){
    //     updated_sept_page_seam_va = 0xffffa00200106000;
    //     std::cout << "updated_sept_page_seam_va: 0x" << updated_sept_page_seam_va << std::endl;

    //     if((updated_sept_page_seam_va != 0) && !is_epte_defined){
    //         ulong epte_seed = 0x80e0000000000000;
    //         std::cout << "considered td_epml4_seam_va: 0x" << updated_sept_page_seam_va << std::endl;
    //         m_VM->createSYMemObject(updated_sept_page_seam_va + 8, 8, 1, 1, epte_seed, "epte");
    //         is_epte_defined = true;
    //     }
    // }
...
int CAnalyze::onBeforeCIESIE(){
    ...
    for(i = 0; i < 2; i++){
        ...
        if(execData->opDetails[i].opmemac.wrmem){
            std::cout << "0mem write: 0x" << std::hex << adr << std::endl;
            ...
            if(isKholeEditAddress(adr)){ /*khole edit mapping*/
                ...
                // seam_pa_va_map.insert({pa, seam_va});
                // seam_va_pa_map.insert({seam_va, pa});
                // std::cout << "2pa: 0x" << std::hex << pa << "\t seam va: 0x" << seam_va << std::endl;
                ...
                //             epml5_mapped_once = true;
                //         else {
                //             updated_sept_page_seam_va = seam_va;
                //         }
                //     }
                //     else{
                //         updated_sept_page_seam_va = seam_va;
                //     }
                //     std::cout << "td sept lvl" << std::dec << sreq->sept_level << "_seam_va: 0x" << std::hex << seam_va << std::endl;
                // }
            }
            // if((adr & ~(0xfffUL)) == td_pml5_seam_va_1){
            //     std::cout << "write to pml5 la-1" << std::endl;
            // }
            // if((adr & ~(0xfffUL)) == updated_sept_page_seam_va){
    ...
    for (auto it : m_VM->m_SYMemObjects)
    {
        ...
        {
            case 1:
                std::cout << "val:0x " << std::hex << it.second->i8 << std::endl;
                break;
            case 4:
                std::cout << "val:0x " << std::hex << it.second->i32 << std::endl;
                break;
            case 8:
                std::cout << "val:0x " << std::hex << it.second->i64 << std::endl;
                break;
        }
    }
...
bool CAnalyze::beginAnalysis(ulong addr) { //Analysis start
    ...
    if(dispatch_count == 0){
        ...
        // exit(0);
        std::cout << "sreq->updated_sept_page:0x" << sreq->updated_sept_page << std::endl;
