    ...
    if(m_regs->regs.rip == map_pa_ret){
        ...
        while(idx < 128){
            ...
            if(keyid_pa_sym == NULL){
                ...
                e_keyhole_keyid_pa_sym_map[idx] = c0;
            }
            else{
            ...
            //check if start_ref_c = end_ref_c
            KVExprPtr e0 = NULL;
            KVExprPtr e1 = NULL;
            ...
            if(!is_ref_c_unmodified){
                ...
                is_ref_c_increased_by_one = a_EFlagsMgr->m_Z3Handler->Z3ConstraintChecking(tmp_constraint2);
                ...
                if(!is_ref_c_increased_by_one){
                    ...
                    std::cout << "correct increase in ref count" << std::endl;
                }
            }
        ...
        a_EFlagsMgr->getLastFlgExpression()->print();
    }
...
// exit(0);
        ...
        // ulong hash_table = 0xffffa00300020e72;
        // MemValue mv2 ;
        // mv2.addr = hash_table;
        // mv2.size = 8 ;
        // mv2.isSymList = false ;
        ...
        // assert(res);
        // ulong hash_table_seed = mv2.i64;
        // std::cout << "sym_buf2_seed: 0x" << std::hex << hash_table_seed << std::endl;
        // m_VM->createSYMemObject(hash_table, 4, 1, 1, hash_table_seed, "hash_t_e");
