...
int is_se = 0;
ulong tdcs_start_seam_va = 0;
ulong to_add_new_symbol = 0;
...
void CAnalyze::track_khole_mappings(){
        ...
        if(execData->opDetails[i].opmemac.wrmem){
            if(adr < 0x300000000000){ /*khole edit mapping*/
                ...
                if(pa == sreq->tdcs_start_pa){
                    ...
                    Checked. ok.*/
                    if(tdcs_start_seam_va == 0){
                        tdcs_start_seam_va = seam_va;
                        std::cout << "tdcs_start_seam_va: 0x" <<tdcs_start_seam_va << std::endl;
                        to_add_new_symbol = 1;
                    }
                    std::cout << "tdcs_start_seam_va: 0x" << seam_va << std::endl;
                }
            }
...
int CAnalyze::onEndOfInsExec(){ //analysis at the end of each instruction
    ...
    if(to_add_new_symbol == 1){
        assert(tdcs_start_seam_va != 0);
        bool res;

        ulong tdcs_binding_state_adr = tdcs_start_seam_va + sreq->tdcs_binding_state_ofst; //1 byte data
        ulong tdcs_binding_state;

        MemValue MV{tdcs_binding_state_adr, 1};
        res = m_VM->readMemory(MV);
        assert(res);
        assert(!MV.bsym);
        tdcs_binding_state = MV.i8;
        std::cout << "tdcs_binding_state: 0x" << tdcs_binding_state << std::endl;
        m_VM->createSYMemObject(tdcs_binding_state_adr, 1, 1, 1, tdcs_binding_state,"bd_state");
        
        std::cout << "tdcs_op_state_ofst: 0x" << sreq->tdcs_op_state_ofst << std::endl;
        ulong tdcs_op_state_adr = tdcs_start_seam_va + sreq->tdcs_op_state_ofst; // 4 bytes data
        ulong tdcs_op_state;
        std::cout << "tdcs_op_state_ofst: 0x" << sreq->tdcs_op_state_ofst << std::endl;
        std::cout << "tdcs_op_state_adr: 0x" << tdcs_op_state_adr << std::endl;

        MemValue MV2{tdcs_op_state_adr, 4};
        res = m_VM->readMemory(MV2);
        assert(res);
        assert(!MV2.bsym);
        tdcs_op_state = MV2.i32;
        std::cout << "tdcs_op_state: 0x" << tdcs_op_state << std::endl;
        m_VM->createSYMemObject(tdcs_op_state_adr, 4, 1, 1, tdcs_op_state,"op_state");
    ...
    if(*(uint8_t *)m_regs->regs.rip == 0xcc) { /*if next rip is int3*/
        ...
        if(*(uint32_t *)int3_adr == ins_seamret){ /*if next sp ins is seamret*/
            ...
            if(execProfile->executionMode != EXEC_MD_SINGLE_PATH_SEDED){
                ...
                // print memory symbols defined
                std::cout << "memory symbols defined: " << std ::endl;
...
int CAnalyze::onBeforeCIESIE(){
    ...
    track_khole_mappings();

    //check if opstate is written
    int i;
    if(tdcs_start_seam_va != 0){
        
        for(i = 0; i < 2; i++){
            //check if memory access address is td_op_state adr
            if(execData->opDetails[i].opmemac.memAddress == (tdcs_start_seam_va + sreq->tdcs_op_state_ofst)){
                std::cout << "accessing op_stae: " << (execData->opDetails[i].opmemac.wrmem ? "WRITE" : "READ") << std::endl;
                assert(!execData->opDetails[i].opmemac.wrmem); //assert if writing to opstate
            }
        }
    }
...
bool CAnalyze::beginAnalysis(ulong addr) { //Analysis start
    ...
    if(dispatch_count == 0){
        ...
        /*servtd_bind, TRover starts at servtd_bind()-------------------------------------------------------*/
        // ulong slot = m_regs->regs.rdx;
        // std::cout << "slot: " << std::hex << slot << std::endl;
        // m_VM->createSYRegObject(x86_64::rdx, 8, 1, 1, slot, "slot");
