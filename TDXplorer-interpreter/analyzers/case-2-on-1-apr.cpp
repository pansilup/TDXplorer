...
void CAnalyze::pageAccessSanitizerSymbolic(){
    ...
    khole_edit_addr_conc = m_Thin->m_EFlagsMgr->ConcretizeExpression(khole_edit_addr, false);
...
int CAnalyze::onEndOfInsExec(){ //analysis at the end of each instruction
    ...
    if(*(uint8_t *)m_regs->regs.rip == 0xcc) { /*if next rip is int3*/
        ...
        uint32_t ins_vmresume = 0x0fc301cc;
        if(*(uint32_t *)int3_adr == ins_vmresume){ /*if next sp ins is seamret*/
            std::cout << ":) ------------------------------- VMRESUME: "<< (m_regs->regs.rax == 0 ? "SUCCESS" : "FAIL") << " : ins count :";
    ...
    else if((m_regs->regs.rip & 0xffffffffff200000) == SEAM_AGENT_CODE){
        // std::cout << "\nagt" << std::endl;
        // uint16_t *opc = (uint16_t *)(m_regs->regs.rip);
        // std::cout << "opc: 0x" << std::hex << (int)(*opc) << " " << (*opc == 0x0f0b) << " " << (*opc == 0x0b0f) << std::endl;
        if(*(uint16_t *)(m_regs->regs.rip) == 0x0b0f){
            std::cout << "\nUD2 End of cur path .... SEAMRET: FAIL" << std::endl;
            endCurrentPath = true;
            endCurrentPathReason = PATH_SEAMRET_FAIL;
            scall_failed_count++;
            std::cout << "\nEnd of cur path ..." << std::endl;
    ...
    if(m_regs->regs.rip == sanitizer_ret_addr){
        std::cout << "\nEnd of SE ..." << std::endl;
        std::cout << "\npath constraints : " << std::endl;
        std::cout << "\nUD2 End of cur path .... SEAMRET: SUCCESS" << std::endl;
        endCurrentPath = true;
        endCurrentPathReason = PATH_SEAMRET_PASS;
        a_EFlagsMgr->PrintConstraint();
        a_EFlagsMgr->SolveConstraints();
...
bool CAnalyze::beginAnalysis(ulong addr) { //Analysis start
    ...
    e5.reset(new EqualExpr(e4));
    m_Thin->m_EFlagsMgr->m_Constraint.insert(e5);*/
