...
struct ExecData {
    ...
    /*to obtain the most recent sym addr*/
    KVExprPtr last_conc_exprptr;
    unsigned long last_conc_expr_val;
    unsigned long last_conc_ins_count;
...
class CAnalyze {
   ...
   public:
    ...
    uint isKholePteSymbolic(ulong rip);
    void pageMapSanitizerSymbolic(ulong reg_idx);
    void pageAccessSanitizerSymbolic();
    void pageMapChecker(uint reg_idx);
    void printSolvedConstraints();
    
    void createSymbolicMemory(ulong addr, ulong size, bool isSigned, bool hasSeed, long conVal, const char *name);
    KVExprPtr readSymMem(unsigned long addr, ulong size, ulong &val);
    void setExecProfileSinglePath();
    void setExecProfileMultiPath();
    KVExprPtr readSymReg(uint reg_idx);
    bool concSymReg(uint reg_idx, ulong val);
    ulong keyidOwnerToKeyid(uint8_t owner);
