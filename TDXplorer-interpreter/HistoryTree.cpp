...
extern unsigned long tpt;
extern unsigned long tp0;
extern unsigned long tp1;
...
bool HistoryManager::endCurrrentExecution(std::set<KVExprPtr> c) {
    ...
    if(!runstack.empty()) {
        ...
        std::cout << "end of path exploration" << std::endl;
        print () ;
...
bool HistoryManager::print(void) {
    ...
    for(auto it_path=all_runpaths.begin() ; 
        ...
        rsn = path_end_reasons[path];
        std::cout << "#" << std::dec << path << " path end reason: ";
        switch (rsn)
        {
        case PATH_CALLRET_WITH_STATUS_SUCCESS:
            std::cout << "PATH_CALLRET_WITH_STATUS_SUCCESS" << std::endl;
            break;
        case PATH_CALLRET_WITH_STATUS_FAIL:
            std::cout << "PATH_CALLRET_WITH_STATUS_FAIL" << std::endl;
            break;
        case PATH_UNSATISFIABLE:
            std::cout << "PATH_UNSATISFIABLE" << std::endl;
            break;
        case PATH_UD2:
            std::cout << "PATH_UD2" << std::endl;
            break;        
        default:
        ...
        /*uncomment this if you want to print symbolic branch trace
    ...
    for(auto it_path=all_runpaths.begin() ; 
        ...
        assert(path < path_end_reasons.size());
        if(path_end_reasons[path] != PATH_UNSATISFIABLE){
            count++;
        }

        /*uncommet if you want to print the symbolic branch trace
    ...
    std::cout << "\npaths considered :" << std::dec << path << std::endl ;
    std::cout << "final valid paths excluding unsatisfiable paths : " << std::dec << count << std::endl;
