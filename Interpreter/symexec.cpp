...
/* Handles the case where the src is sym
bool SymExecutor::process_bt(VMState *vm, InstrInfoPtr &infoptr) {
    auto &vecOI = infoptr->vecOI;
    OprndInfoPtr &oidst = vecOI[0];
    OprndInfoPtr &oisrc = vecOI[1];
    KVExprPtr e0, e1, e2, er, temp1, temp2, temp3;
    long v1, v2 ;
    bool res ;
    ulong conc_v;
    bool is_src_conc = false;
    ...
    if(oisrc->symb) {
        ...
        res = oisrc->getSymValue(e0) ;
        assert(res);
        std::cout << "src is symbolic: ";
        e0->print();
        std::cout << std::endl;
        std::cout.flush();
        conc_v = (ulong)vm->m_EFlagsMgr->ConcretizeExpression(e0);
        std::cout << "conc_v: " << std::dec << conc_v << std::endl;
        is_src_conc = true;
        // e1.reset(new ConstExpr(conc_v, oidst->size, 0)) ;
    ...
    if(oidst->symb) {
        ...
        res = oidst->getConValue(v2) ;
        assert(res);
        //now handle the case where the src has been concretized
        if(is_src_conc){
            // assert(oidst->opty == OPTY_REGCON);  //as of now We expect the dst to be conc reg val
            ...
            eflags.cf = (v2 >> conc_v) & 0x1;
            // eflags.cf = (v2 & ~(1UL << v1)) >> v1;
...
bool SymExecutor::process_btr(VMState *vm, InstrInfoPtr &infoptr) {
    ...
    if(oisrc->symb || oidst->symb) {
        ...
        // std::cout << "cf expr: ";
        // er->print();
        // temp4.reset(new AndExpr(e2, temp2));
        // er.reset(new ShrExpr(temp4, e1));
        // er.reset(new BtExpr(e1, e2, oidst->size, 0)) ;
...
bool SymExecutor::process_xadd(VMState *vm, InstrInfoPtr &infoptr) {
    ...
    if (oisrc1->symb && oisrc2->symb) {
        ...
        // bool sf;
        // std::cout << "oisrc1->size: " << oisrc1->size <<std::endl;
        // std::cout << "oisrc2->size: " << oisrc2->size <<std::endl;
        // unsigned long rflags = 0xceedceed;

        // if(oisrc1->size == 2)
        // std::cout << "\nv1:" << std::hex << v1 << " v2:" << v1 << " rflags:" << rflags << std::endl;              
        // long v1_1;
        ...
        //               "movq (%%rbx), %%rcx; \n\t"
        //               "movq %%rcx, %1"
        //               :"=m"(rflags), "=m"(v1_1): "m"(v2), "m"(v1): "memory", "%rax", "%rbx", "%rcx");
        
        // std::cout << "v1_1:" << std::hex << v1_1 << std::endl;              
        // std::cout << "v1:" << std::hex << v1 << " v2:" << v1 << " rflags:" << rflags << std::endl;              
        // // assert(0);

        // temp = v1 + v2;
        // vm->setFlagBit(x86_64::cf, flagclr); //chk
        // vm->setFlagBit(x86_64::zf, (temp == 0) ? flagset : flagclr);

        // if(oisrc1->size == 1){ 
        //     sf = ((temp & 0x80) == 1);
        // }
        // else if(oisrc1->size == 4){ 
        //     sf = ((temp & 0x80000000) == 1);
        // }
        // else if(oisrc1->size == 8){ 
        //     sf = ((temp & (1UL << 63)) == 1);
        // }
        // else {
        //     assert(0);
        // }
        // vm->setFlagBit(x86_64::sf, sf ? flagset : flagclr);
        // vm->setFlagBit(x86_64::of, flagclr); // ?
        // vm->setFlagBit(x86_64::af, flaguc); // ?
        // bool pf = std::bitset<8>(temp & 0xff).count() % 2 == 0; // Even number of 1 bits -> even parity
        // vm->setFlagBit(x86_64::pf, pf ? flagset : flagclr);
...
bool SymExecutor::process_test(VMState *vm, InstrInfoPtr &infoptr) {
    ...
    if (oisrc1->symb && oisrc2->symb) {
        ...
        // bool pf = std::bitset<8>(temp & 0xff).count() % 2 == 0; // Even number of 1 bits -> even parity
        // vm->setFlagBit(x86_64::pf, pf ? flagset : flagclr);
        // vm->setFlagBit(x86_64::cf, flagclr);
        // vm->setFlagBit(x86_64::of, flagclr);
...
bool SymExecutor::process_cmp(VMState *vm, InstrInfoPtr &infoptr) {
    ...
    if (oisrc1->symb && oisrc2->symb) {
        ...
        // std::cout << "rflags:" << rflags << " src:0x" << v2 << " dst:0x" << v1 << std::endl;       
        // switch (oidst->size) {
        //     case 4: {
        //         std::cout << "operation size 4" << std::endl;       
        //         uint32_t a = (uint32_t)v1;
        //         uint32_t b = (uint32_t)v2;
        //         std::cout << std::hex << "rflags:" << rflags << " src:0x" << (int)b << " dst:0x" << (int)a << std::endl;       
        //         asm volatile (  "pushfq; \n\t"
        //                         "mov %1, %%ecx; \n\t"
        //                         "mov %2, %%edx; \n\t"
        //                         "cmp %%edx, %%ecx; \n\t" //(ecx - edx) and set flags
        //                         "pushfq; \n\t"
        //                         "popq %0; \n\t"
        //                         "popfq; \n\t"
        //                         :"=m"(rflags): "m"(a), "m"(b): "%r11", "%ecx", "%edx");

        //         std::cout << "rflags:" << rflags << std::endl;       
        //     } break;
        //     case 1: {
        //         std::cout << "operation size 1" << std::endl;       
        //         uint8_t a = 0; //(uint8_t)v1;
        //         uint8_t b = 1; //(uint8_t)v2;
        //         ulong flg_start = 0;
        //         std::cout << std::hex << "rflags:" << rflags << " src:0x" << (int)b << " dst:0x" << (int)a << std::endl;       
        //         asm volatile (  "movq $0xbeed, %%r11; \n\t"
        //                         "pushfq; \n\t"
        //                         "popq %1; \n\t"
        //                         "pushfq; \n\t"
        //                         "mov %2, %%cl; \n\t"
        ...
        //                         "popq %0; \n\t"
        //                         "popfq; \n\t"
        //                         :"=m"(rflags), "=m"(flg_start): "m"(a), "m"(b): "%r11", "%cl", "%dl", "%r11");
        //         std::cout << "rflags:" << rflags << " flg_start:" << flg_start << std::endl;   
        //         assert(0);
        //     } break;
        //     default: {
        //         assert(0);
...
//         res = oisrc1->getConValue(v1);
//         assert(res);
//         e1.reset(new ConstExpr(v1, oisrc1->size, 0));
//     }
...
//         res = oisrc2->getConValue(v2);
//         assert(res);
//         e2.reset(new ConstExpr(v2, oisrc1->size, 0));
//     }

//     KVExprPtr oe(new Shl_SalExpr(e1, e2));
//     res = oidst->setSymValue(vm, oe);
...
ulong SymExecutor::getSegRegVal(VMState* vm, DAPIInstrPtr& I)
{
    int r_id;
    /* check if Insn uses gs as (es, ds) base in mem access, if yes, get gsbase first */
    std::set<RegisterAST::Ptr> regrd = I.get()->getOperation().implicitReads();
    if (regrd.size() != 0)
    {
        for (auto it : regrd)
...
bool SymExecutor::_parseOperand_RX(VMState *vm, DAPIInstrPtr& I, OprndInfoPtr &oi) {
    ...
    if (O->isRead()) {
        ...
        ulong fsgs_base = getSegRegVal(vm, I); 
...
bool SymExecutor::parseOperands(VMState *vm, InstrInfo *info, bool isSymList) {
    ...
    // std::cout << "#of operands: " << oprands.size() << std::endl;
    // exit(0);
    ...
    for (auto O : oprands) {
        ...
        // std::cout << "O->isRead() :" << O.isRead() <<std::endl;
        // std::cout << "O->isWrite() :" << O.isWritten() <<std::endl;
